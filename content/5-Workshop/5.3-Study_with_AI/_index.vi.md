...
### c. Kiểm tra Extension Gate (checkGateStatus — mỗi 3 giây)
...
**Bước 0 — Trích xuất metadata (extractor.js — MAIN world):**
...
*`parseAiResponse()`:* Dùng regex `/{"result"\s*:\s*"(ALLOW|BLOCK)"[\s\S]*?}/i` để trích JSON an toàn. Nếu regex fail → tìm từ khóa ALLOW/BLOCK trong text (fallback). Nếu không tìm thấy gì → mặc định **BLOCK** (an toàn nhất).
...
### i. Phân loại Web cứng (Hard-block) và Trang web bất kỳ
...
### k. Xử lý Strike & Kết thúc phiên phía Server (studySessionService.mjs)

**Ghi Strike (`POST /strike`):**
