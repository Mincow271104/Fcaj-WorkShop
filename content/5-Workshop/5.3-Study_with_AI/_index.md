...
6. `parseResponse()` uses regex `/{[\s\S]*}/` to extract JSON from the response — safe even with extraneous markdown code blocks.
...
*`parseAiResponse()`:* Uses regex `/{"result"\s*:\s*"(ALLOW|BLOCK)"[\s\S]*?}/i` to safely extract JSON. If regex fails → searches for keywords ALLOW/BLOCK in text (fallback). If nothing found → defaults to **BLOCK** (safest).
